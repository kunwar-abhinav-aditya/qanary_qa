...
public class MeaningCloudNed extends QanaryComponent {
	...
	public QanaryMessage process(QanaryMessage myQanaryMessage) throws Exception {
		...
		logger.info("Question {}", myQuestion);
		 ...
		 try {
				File f = new File("qanary_component-NED-MeaningCloud/src/main/resources/questions_NED-MeaningCloud.txt");
				f.createNewFile(); 
...
//			            	}
//				    	flag=0;
//				    	//flag has to be 1 for proper caching but for local temporary output keeping flag 0
//				    	
//				    	break;	
//				    }
//				   
...
//				{
		...
		try {
						...
						if (formObject.has("variant_list")) {
							...
							for (int i = 0; i < jsonArray.length(); i++) {
								...
								if(link!=null && link.contains("wikipedia")) {
									finalUrl = "http://dbpedia.org/resource"+link.substring(28).replace(" ", "_");
								}
			...
			BufferedWriter buffWriter = new BufferedWriter(new FileWriter("qanary_component-NED-MeaningCloud/src/main/resources/questions_NED-MeaningCloud.txt", true));
	        Gson gson = new Gson();
